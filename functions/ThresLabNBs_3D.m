function [TL Stats_table] = ThresLabNBs_3D(toThreshold,parameters,varargin)
        ...
        %26/05/19 test to run active contours in 3d volume
%         mask = ones(size(toThreshold));
%         BW = zeros(size(toThreshold));
%         %mask(2:size(mask,1)-1,2:size(mask,2)-1)=1;
%         for z=1:size(toThreshold,3)
%             BW(:,:,z)=imbinarize(toThreshold(:,:,z),Level);
%         end
%         T = activecontour(toThreshold,BW,100);  
        ...
        Stats_table = regionprops('table',TL,'Area','Centroid','PixelIdxList');
